...
- **MCP 扩展**:接入 MCP 服务器生态(文件系统 / GitHub / 浏览器 / 数据库…),工具自动出现在对话里,见 [docs/MCP.md](docs/MCP.md)
- **本机优先**:对话与配置留在你的电脑里;不依赖任何在线服务(公网部署只是可选项)
- **PWA**:手机、平板、桌面都能「添加到主屏幕」,体验接近原生 App
- **会自我进化**(进阶):它能在后台自己练习、自己改进自己的工作方式 —— 见下一节

## 自我进化:改进 Agent,而不是训练模型

Qball 可以在后台**自己练习、自己变强**:给它真实的工具和任务,让它反复干活;失败就反思,
把经验沉淀成行为规则与技能,甚至修改自己的代码 —— 全程不需要 GPU,也**从不训练模型权重**。

> **模型永远是你配置的那个云端 API,一个字节都不会变;被改进的是 Agent 本身**
> (行为规则、技能库、工作方式、代码)。

每一代改进都要过三道安全门——**得分门禁 / 路径白名单 / 全套自动化测试**——
全绿才自动采纳(git 版本可随时一键回滚)。

它还会**把自己当成用户**做体验改进:小到按钮/文案由它自己改,大建议写进「体验改进报告」,
交给开发助手实现;采纳与新建议会弹 Windows 通知,面板里能直接翻看它的技能库与报告。
原理、门禁与使用方式见 **[docs/EVOLUTION.md](docs/EVOLUTION.md)**。
